...
log("=== Iniciando actualización mensual (pipeline v1.3.0) ===")
...
#   Papaya 526 (Apricots) → 600 (Papayas) | Aceituna 568 (Melones) → 260 (Olives)
#   + Jengibre (720) y Aji_Rocoto (401) que faltaban | − Quinua/Cebada (no están en el dashboard)
CULTIVOS = {
    ...
    260:"Aceituna",552:"Arandano",720:"Jengibre",401:"Aji_Rocoto",15:"Trigo",
    176:"Frijol",181:"Haba",122:"Camote",366:"Alcachofa",600:"Papaya",
}
PALABRA_CLAVE_ITEM = {
    "Cana_azucar":"sugar cane","Papa":"potato","Arroz":"rice","Platano":"banana",
    "Maiz":"maize","Palma_aceitera":"oil palm","Yuca":"cassava","Uva":"grape",
    "Palta":"avocado","Mandarina":"mandarin","Cebolla":"onion","Pina":"pineapple",
    "Naranja":"orange","Maiz_choclo":"green corn","Limon":"lemon","Cafe":"coffee",
    "Cacao":"cocoa","Esparrago":"asparagus","Tomate":"tomato","Mango":"mango",
    "Aceituna":"olive","Arandano":"blueberr","Jengibre":"ginger","Aji_Rocoto":"chillies",
    "Trigo":"wheat","Frijol":"bean","Haba":"broad bean","Camote":"sweet potato",
    "Alcachofa":"artichoke","Papaya":"papaya",
}
...
# Validación anti-error de mapeo: si un código apunta a otro cultivo, ABORTAR.
errores = []
for cultivo, item in peru_largo.groupby('Cultivo')['Item'].first().items():
    clave = PALABRA_CLAVE_ITEM.get(cultivo, '')
    if clave and clave not in str(item).lower():
        errores.append(f"  ✗ {cultivo}: código apunta a '{item}' (se esperaba '{clave}')")
if errores:
    log("ERROR DE MAPEO FAOSTAT — actualización abortada:\n" + "\n".join(errores))
    raise SystemExit(1)
log("✓ Validación de mapeo cultivo↔ítem FAOSTAT: OK")
...
log(f"✓ Producción actualizada: {len(peru_largo)} registros, {peru_largo['Cultivo'].nunique()} cultivos")
...
region_cultivo = {
    ...
    'Jengibre':'Junin','Aji_Rocoto':'Arequipa',
}
...
# v1.3.0 — Clima de predicción: ÚLTIMOS 12 MESES (ventana móvil), no el año
# calendario parcial. Antes se sumaba la lluvia de enero–hoy contra modelos
# entrenados con totales anuales completos, sesgando sistemáticamente el riesgo.
fecha_corte = clima.index.max()
ventana_inicio = fecha_corte - pd.DateOffset(months=12) + pd.DateOffset(days=1)
clima_12m = clima[clima.index >= ventana_inicio]
dias_ventana = (fecha_corte - ventana_inicio).days + 1
clima_actual = clima_12m.groupby('region').agg(
    temp_media=('temp_media','mean'),
    temp_min=('temp_min','mean'),
    precipitacion_total=('precipitacion_mm','sum'),
    humedad_media=('humedad_%','mean')
).reset_index()
# Normalizar la precipitación a 365 días exactos por si la ventana difiere
clima_actual['precipitacion_total'] = clima_actual['precipitacion_total'] * (365.0 / dias_ventana)
log(f"✓ Clima de predicción: últimos 12 meses ({ventana_inicio.date()} → {fecha_corte.date()})")

anio_min = int(df_cruce['Año'].min())
anio_max = int(df_cruce['Año'].max())
...
for cultivo, region in region_cultivo.items():
    fila = {
        ...
        'Riesgo_%': None,
        'Nivel': 'N/D',
        'Media_historica_ton': round(medias[cultivo]) if cultivo in medias else None,
    }
    info = modelos.get(cultivo)
    cr = clima_actual[clima_actual['region']==region]
    if info is not None and not cr.empty:
        X = info['scaler'].transform(cr[features])
        prob = info['model'].predict_proba(X)[0]
        prob_riesgo = prob[1] if len(prob) > 1 else 0
        fila['Riesgo_%'] = round(prob_riesgo*100,1)
        fila['Nivel'] = 'ALTO' if prob_riesgo>0.6 else 'MEDIO' if prob_riesgo>0.35 else 'BAJO'
    riesgos.append(fila)

riesgo_df = pd.DataFrame(riesgos).sort_values('Riesgo_%', ascending=False, na_position='last')
riesgo_df.to_csv(os.path.join(PROYECTO, 'riesgo_cosecha_actual.csv'), index=False)
n_ml = riesgo_df['Riesgo_%'].notna().sum()
log(f"✓ Riesgos calculados: {n_ml} cultivos con modelo ML, {len(riesgo_df)-n_ml} en N/D "
    f"(media histórica unificada {anio_min}–{anio_max} para los 30 cultivos)")

# --- 5. Regenerar dashboard ---
log("Regenerando dashboard...")
con_ml = riesgo_df.dropna(subset=['Riesgo_%']).copy()
colores = {'ALTO':'#E74C3C','MEDIO':'#F39C12','BAJO':'#27AE60'}
con_ml['Color'] = con_ml['Nivel'].map(colores)
riesgo_sorted = con_ml.sort_values('Riesgo_%', ascending=True)

fig = make_subplots(
    rows=1, cols=2,
    subplot_titles=('Riesgo de mala cosecha — últimos 12 meses de clima','Distribución por nivel'),
...
conteo = con_ml['Nivel'].value_counts()
fig.add_trace(go.Pie(
    labels=conteo.index, values=conteo.values,
    marker_colors=[colores.get(n,'#999') for n in conteo.index], hole=0.4,
), row=1, col=2)

mes_actual = datetime.now().strftime('%B %Y')
fig.update_layout(
    title=f'🌾 Riesgo de Mala Cosecha — Perú {mes_actual} (indicador experimental, actualizado automáticamente)',
    height=600, template='plotly_white', showlegend=False
)
fig.write_html(os.path.join(PROYECTO, 'dashboard_riesgo_cosecha.html'))

log(f"✓ Dashboard actualizado — {n_ml} cultivos con ML de {len(riesgo_df)} totales")
