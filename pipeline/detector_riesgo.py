...
# --- Predecir riesgo para el último año completo disponible ---
clima = pd.read_csv(os.path.join(PROYECTO, 'clima_peru_completo.csv'),
                    index_col='fecha', parse_dates=True)
# v1.3.0: año dinámico (antes estaba hardcodeado en 2024)
anio_prediccion = int(clima.index.year.max())
print(f"Calculando riesgos {anio_prediccion}...")
clima_2024 = clima[clima.index.year == anio_prediccion].groupby('region').agg(
...
region_cultivo = {
    ...
    'Papaya': 'Loreto', 'Jengibre': 'Junin', 'Aji_Rocoto': 'Arequipa',
}
...
for cultivo, modelo_info in modelos.items():
    ...
    if clima_region.empty:
        continue
    X_scaled = modelo_info['scaler'].transform(clima_region[features])
...
riesgo_df.to_csv(os.path.join(PROYECTO, f'riesgo_cosecha_{anio_prediccion}.csv'), index=False)
...
fig = make_subplots(
    rows=2, cols=2,
    subplot_titles=(
        f'Riesgo de mala cosecha por cultivo {anio_prediccion}',
...
fig.update_layout(
    title=dict(
        text=f'🌾 Sistema de Detección de Riesgo de Mala Cosecha — Perú {anio_prediccion}',
        font=dict(size=18)
    ),
    height=900,
    showlegend=True,
...
print((f"\n--- RESUMEN RIESGO {anio_prediccion} ---"))
